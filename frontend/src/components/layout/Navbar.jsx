...
import { HiOutlineHome, HiOutlineMenu, HiX, HiOutlineUser, HiOutlineLogout } from 'react-icons/hi'
            ...
            {[
              ...
              <Link
                ...
                className={`text-sm font-medium transition-colors ${
                  isActive(to) ? 'text-gold' : 'text-white/70 hover:text-white'
                }`}
              >
                {label}
              </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              ...
              <>
                ...
                <Link
                  to="/register"
                  className="flex items-center gap-1.5 btn-gold px-4 py-2 rounded-lg text-sm"
                >
          ...
          <div className="md:hidden border-t border-white/10 py-4 space-y-1">
            ...
            <div className="pt-3 flex flex-col gap-2">
              {user ? (
                ...
                <>
                  <Link to="/login" onClick={() => setIsOpen(false)} className="text-center py-2.5 text-sm text-white/70 border border-white/20 rounded-lg">Sign In</Link>
                  <Link to="/register" onClick={() => setIsOpen(false)} className="text-center py-2.5 text-sm btn-gold rounded-lg">List Property</Link>
                </>
              )}
            </div>
          </div>
